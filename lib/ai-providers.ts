...
export function supportsImageInput(modelId: string): boolean {
    ...
    // Qwen3.6 series (qwen3.6, qwen3.6-plus, qwen3.6-flash) natively support image input
    ...
    // Gemma text models (not vision variants)
    // Gemma4 series (gemma4, gemma4-plus, gemma4-flash) natively support image input
    if (
        lowerModelId.includes("gemma") &&
        !hasVisionIndicator &&
        !lowerModelId.includes("gemma4")
    ) {
        return false
    }
